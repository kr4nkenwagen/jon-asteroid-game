...
class ui_score(entity):
    def __init__(self):
        ...
        self.frame = pygame.Rect((SCREEN_WIDTH // 2) - UI_SCORE_WIDTH // 2, SCREEN_HEIGHT - UI_OFFSET + UI_SCORE_HEIGHT, UI_SCORE_WIDTH, UI_SCORE_HEIGHT)
        self.value_rect = pygame.Rect(self.frame.x, self.frame.y, 0, self.frame.height)
        self.player = None
        self.value = 0
        self.update_value = 0
        self.player_level = 0
    ...
    def update(self):
        ...
        if self.update_value > 0:
            diff = pygame.math.lerp(0, self.update_value, min(max(self.game.dt, 0), 1))
    ...
    def draw(self):
       ...
       pygame.draw.rect(self.game.screen, "blue", pygame.Rect(self.value_rect.x + self.value_rect.width, self.value_rect.y, (self.update_value / LEVEL_LIMIT) * UI_SCORE_WIDTH, self.value_rect.height))
