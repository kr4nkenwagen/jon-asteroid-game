...
class asteroid_spawner(entity):
    ...
    def create_asteroid_velocity(self, start):
        if self.player == None:
            return (pygame.Vector2(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2) - start).normalize() * random.randint(ASTEROID_MIN_SPEED, ASTEROID_MAX_SPEED)
        return (self.player.position - start).normalize() * random.randint(ASTEROID_MIN_SPEED, ASTEROID_MAX_SPEED)
